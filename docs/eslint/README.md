...
JSX-only components, one-way layer imports, package ownership, env/browser facades, query-key
builders, i18n copy discipline, and justified client boundaries.

## Where the code lives

| Piece                                 | Path                                               |
| ------------------------------------- | -------------------------------------------------- |
| Plugin entry (registers all 13 rules) | `eslint/architecture-plugin.mjs`                   |
...
- `eslint/architecture.config.mjs` registers the plugin under the key `frontend-architecture`
  for `src/**/*.{ts,tsx}` and enables 12 of the 13 rules at `error`. It also supplies the
