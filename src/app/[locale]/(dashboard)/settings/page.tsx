...
export async function generateMetadata(props: LocaleRouteProps): Promise<Metadata> {
  ...
  const t = await getServerTranslations({ locale, namespace: I18N_NAMESPACES.settings });

  return { title: buildPageTitle(t('title')) };
}

export default async function SettingsPage(props: LocaleRouteProps): Promise<ReactElement> {
  const { locale } = await props.params;
  if (!isSupportedLocale(locale)) {
    appNotFound();
  }
